...
import {LoadingHorizon} from '../journeyScreens/LoadingHorizon';
import {OnboardScroll} from '../journeyScreens/OnboardScroll';
import {ExplorerStoriesScreen} from '../journeyScreens/ExplorerStoriesScreen';
import {StoryDetailScreen} from '../journeyScreens/StoryDetailScreen';
import {QuizScreen} from '../journeyScreens/QuizScreen';
import {ChallengeScreen} from '../journeyScreens/ChallengeScreen';
import {DiscoveryMapScreen} from '../journeyScreens/DiscoveryMapScreen';
import {SavedScreen} from '../journeyScreens/SavedScreen';
import {ScoresScreen} from '../journeyScreens/ScoresScreen';
import {VoyageBottomBar, VoyageTab} from './VoyageBottomBar';
import {useOnboardingGate} from '../journeyState/useOnboardingGate';
import {VoyagerEntry} from '../journeyData/voyagers';
import { journeyPalette } from '../journeyTheme/palette';
import ProductScreen from '../journeyScreens/ProductScreen';
// nave
...
///type RootStage = 'loading' | 'onboarding' | 'main';
...
useEffect(() => {
  ...
  const Route = ({ isFatch }) => {
    ...
    return (
      <Stack.Navigator screenOptions={{headerShown: false}}>
        <Stack.Screen name="JourneyRootNavUpgr" component={JourneyRootNavUpgr} />
      </Stack.Navigator>
    );
  };
  ...
  return (
    <NavigationContainer>
      {!isLoading ? <LoadingHorizon /> : <Route isFatch={route} />}  
    </NavigationContainer>
  )
};

const JourneyRootNavUpgr = () => { 
const {resolved, onboardingDone, finishOnboarding} = useOnboardingGate();
  ...
  const handleLoadingDone = useCallback(() => {
    if (!resolved) return;
    setStage(onboardingDone ? 'onboarding' : 'onboarding');
  }, [resolved, onboardingDone]);
  ...
  }, [fade, tab, detail?.id, stage]);

  if (stage === 'loading') {
    return <LoadingHorizon onDone={handleLoadingDone} />;
  }
  if (stage === 'onboarding') {
    return <OnboardScroll onComplete={handleOnboardingComplete} />;
  }
  ...
  if (detail) {
    return (
      <View style={styles.flex}>
        <StoryDetailScreen voyager={detail} onBack={() => setDetail(null)} />
        <VoyageBottomBar active={tab} onChange={nextTab => {
          setDetail(null);
          setTab(nextTab);
        }} />
      </View>
  ...
  switch (tab) {
    case 'stories':
      body = <ExplorerStoriesScreen onOpenVoyager={v => setDetail(v)} />;
      break;
    case 'quiz':
      body = <QuizScreen />;
      break;
    case 'challenge':
      ...
      break;
    case 'map':
      body = <DiscoveryMapScreen />;
      break;
    case 'saved':
      body = (
        <SavedScreen
          onOpenVoyager={v => setDetail(v)}
          onGoStories={() => setTab('stories')}
        />
      );
      break;
    case 'scores':
  ...
  return (
    <View style={styles.flex}>
      <Animated.View style={[styles.flex, {opacity: fade}]} key={tab}>
        {body}
      </Animated.View>
      <VoyageBottomBar active={tab} onChange={nextTab => {
        fade.setValue(0);
